...
public class PaginationSupport<T> {
	...
	public ArrayList<Integer> page(int totalPages, int currentPage) {
		...
		if (totalPages < (5 + (adjacents * 2))) { // not enough links to make it
			...
			if ((totalPages - (adjacents * 2) > currentPage)
					&& (currentPage > (adjacents * 2))) { // in th
				writeNumberedLinks(1, 1, result);
				writeElipsis(result);
				writeNumberedLinks(currentPage - adjacents - 1, currentPage
						+ adjacents, result);
				writeElipsis(result);
				...
				writeNumberedLinks(1, 1, result);
				writeElipsis(result);
				writeNumberedLinks(totalPages - (2 + (adjacents * 2)),
						totalPages, result);
			}
		}
		return result;
	 ...
	 * @param result
	 */
	private void writeNumberedLinks(int i, int lastIndex,
